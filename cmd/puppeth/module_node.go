...
{{end}}
RUN \
  echo 'gprog --cache 512 init /genesis.json' > gprog.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.originelladium/keystore/ && cp /signer.json /root/.originelladium/keystore/' >> gprog.sh && \{{end}}
	echo $'exec gprog --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gprog.sh

ENTRYPOINT ["/bin/sh", "gprog.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.originelladium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.proghash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.originelladium"],
		ethashdir:  infos.volumes["/root/.proghash"],
		port:       port,
